...
export default class view extends Component {
    ...
    componentDidMount(){
      ...
      fetch(`http://localhost:3001/patients/Update/${id}`).then(res=>res.json()).then((result)=>this.setState({
      name:result.name,
      email:result.email,
    ...
    handleSubmit(e){
      e.preventDefault();
      fetch(`http://localhost:3001/patients/Update/${this.state.id}`,{
            headers: {
    ...
    <div style={{display:"flex",marginBottom:"20px"}}>
      ...
      <div className="form-group">
          ...
          <div className="col-sm-10">
            <input type="text" value={this.state.name} onChange={this.handleChange} className="form-control" id="name" placeholder="Enter name" name="name"/>
          </div>
        </div>
        <div className="form-group">
          <label className="control-label col-sm-2" >Email:</label>
          <div className="col-sm-10">
            <input type="email" value={this.state.email} onChange={this.handleChange} className="form-control" id="email" placeholder="Enter email" name="email"/>
          </div>
        </div>
        <div className="form-group">
          <label className="control-label col-sm-2" >Password:</label>
          <div className="col-sm-10">          
            <input type="password" value={this.state.password} onChange={this.handleChange} className="form-control" id="pwd" placeholder="Enter password" name="password"/>
          </div>
        </div>
        <div className="form-group">
          <label className="control-label col-sm-2" >Phone:</label>
          <div className="col-sm-10">          
            <input type="number" value={this.state.phone} onChange={this.handleChange} className="form-control" id="phone" placeholder="Enter Phone" name="phone"/>
          </div>
        </div>
        <div className="form-group">   
